...
class TextProcessorFluentApi {
    ...
    extractPeopleData() {
        ...
        const onlyPeople = this.#content.match(matchPerson);
        this.#content = onlyPeople;
        return this;
    }

    divideTextInColumns() {
        const splitRegex = evaluateRegex(/,/);
        this.#content = this.#content.map(person => person.split(splitRegex));
        return this
    }

    removeEmptyCharacters() {
        const trimSpaces = evaluateRegex(/^\s+|\s+$|\n/g);
        this.#content = this.#content.map(person => person.map(field => field.replace(trimSpaces, '')));
        return this;
    }

    mapPerson() {
        // passa array de itens no construtor 
